...
public class PrivacySettingsTab implements ISettingsTab {
	...
	public void createInputs(Composite tabComposite,
		...
		val = BrowserUtil.getPreferenceValue(preferences, PreferenceNames.ACCEPT_COOKIES_FROM_SITES); // Uses same preference with btnAcceptCookiesFromSites
	...
	public void addValuesToList(Profile profile) throws Exception {
		...
		preferences.add(new BrowserPreference(PreferenceNames.ACCEPT_COOKIES_FROM_SITES, getSelectedCookieAcceptPolicy()));
		preferences.add(new BrowserPreference(PreferenceNames.CLEAR_HISTORY_ON_CLOSE, btnClearHistoryWhenFirefoxCloses.getSelection() ? "true" : "false"));
		preferences.add(new BrowserPreference(PreferenceNames.SUGGEST_HISTORY, btnSuggestHistory.getSelection() ? "true" : "false"));
	...
	private String getSelectedCookieAcceptPolicy() {
		...
		if (selectionIndex > -1 && cmbAcceptThirdPartyCookies.getItem(selectionIndex) != null && cmbAcceptThirdPartyCookies.getData(selectionIndex + "") != null) {
	...
	private String getSelectedCookieLifetimePolicy() {
		...
		if (selectionIndex > -1 && cmbKeepCookiesUntil.getItem(selectionIndex) != null && cmbKeepCookiesUntil.getData(selectionIndex + "") != null) {
